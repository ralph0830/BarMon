...
// 바몬 전체 조회 API
app.get('/api/barmon', async (req: Request, res: Response) => {
  const result = await db.select().from(barmon);
  res.json(result);
});

// 바몬 시드 데이터 삽입 (최초 1회)
app.post('/api/barmon/seed', async (req: Request, res: Response) => {
  try {
    for (const item of barmonSeed) {
      await db.insert(barmon).values(item).onConflictDoNothing();
    }
    res.json({ ok: true });
  } catch (e) {
...
// 바몬 소환(스캔) API
app.post('/api/barmon/scan', (req: Request, res: Response) => {
  (async () => {
    ...
    // DB에 중복 없으면 삽입
    try {
      await db.insert(barmon).values(newBarMon).onConflictDoNothing();
      res.json(newBarMon);
    } catch (e) {
